...
```{r}
flights_df <- sqldf("SELECT fid,flights_df.rid,`date`,airline,aircraft, altitude, heavy, aid AS origin FROM flights_df
...
```{r}
num_rows <- nrow(flights_df)

# Create a vector with values from 1 to the number of rows
new_fid <- 1:num_rows

# Assign the new_fid vector to the fid column in flights_df
flights_df$fid <- new_fid
...
#```{r}
#flights_df <- sqldf("SELECT f.fid,f.rid,f.`date`,f.airline,f.aircraft, f.altitude, f.heavy, a.aid AS origin 
#FROM flights_df f
#JOIN airport_df a ON f.rid = a.rid")
...
```{r}
num_rows <- nrow(strikes_df)

# Create a vector with values from 1 to the number of rows
new_sid <- 1:num_rows

# Assign the new_fid vector to the sid column in s
strikes_df$sid <- new_sid
strikes_df
```
